...
// 🎫 Systèmes externes
import { handleTicketButtons } from './commands/ticket/ticketpanel.js'; // ✅ corrigé
...
// 👋 Bienvenue
import { welcomeEvent } from './commands/welcome.js';
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isModalSubmit()) {
    ...
    // 🎫 Modals tickets & candidatures — EN PREMIER
    ...
    if (interaction.customId === 'suggestion_modal') {
      ...
      try {
        ...
        const suggestionData = {
          id: suggestionId, messageId: suggestionMsg.id, channelId: suggestionChannel.id,
          authorId: interaction.user.id, title, content: `**${title}**\n\n${description}`,
          status: 'pending', upvotes: new Set(), downvotes: new Set(),
          createdAt: Date.now(), staffResponse: null
        };
  ...
  switch (interaction.customId) {
    case 'suggestion_submit': {
      ...
      modal.addComponents(
        new ActionRowBuilder().addComponents(
          new TextInputBuilder().setCustomId('suggestion_title').setLabel('Titre')
            .setStyle(TextInputStyle.Short).setPlaceholder('Ex: Ajouter un salon gaming')
            .setRequired(true).setMaxLength(100)
        ),
        new ActionRowBuilder().addComponents(
          new TextInputBuilder().setCustomId('suggestion_description').setLabel('Description')
            .setStyle(TextInputStyle.Paragraph).setPlaceholder('Explique ta suggestion...')
    ...
    case 'suggestion_top': {
      ...
      const topEmbed = new EmbedBuilder()
        .setTitle('🏆 Top Suggestions').setColor('#FFD700')
    ...
    case 'suggestion_rules': {
      const rulesEmbed = new EmbedBuilder()
        .setTitle('📜 Règles des suggestions').setColor('#57F287')
  ...
  if (interaction.customId === 'suggest_view_pending') {
    ...
    const pending = Array.from(suggestCmd.getAllSuggestions().values()).filter(s => s.status === 'pending');
    if (pending.length === 0) return interaction.reply({ content: '📋 Aucune suggestion en attente.', ephemeral: true });
    const list = pending.slice(0, 10).map(s => `**#${s.id}** - ${s.title?.substring(0, 50) || s.content.substring(0, 50)}... (👍 ${s.upvotes.size} | 👎 ${s.downvotes.size})`).join('\n');
    return interaction.reply({ embeds: [new EmbedBuilder().setColor('#FEE75C').setTitle('🟡 Suggestions en attente').setDescription(list).setFooter({ text: `Total: ${pending.length}` })], ephemeral: true });
  }

  if (interaction.customId === 'suggest_view_accepted') {
    if (!suggestCmd) return interaction.reply({ content: '❌ Système non disponible.', ephemeral: true });
    const accepted = Array.from(suggestCmd.getAllSuggestions().values()).filter(s => s.status === 'accepted');
    if (accepted.length === 0) return interaction.reply({ content: '✅ Aucune suggestion acceptée.', ephemeral: true });
    const list = accepted.slice(0, 10).map(s => `**#${s.id}** - ${s.title?.substring(0, 50) || s.content.substring(0, 50)}... (👍 ${s.upvotes.size})`).join('\n');
    return interaction.reply({ embeds: [new EmbedBuilder().setColor('#57F287').setTitle('✅ Suggestions acceptées').setDescription(list).setFooter({ text: `Total: ${accepted.length}` })], ephemeral: true });
  }

  if (interaction.customId === 'suggest_top') {
    if (!suggestCmd) return interaction.reply({ content: '❌ Système non disponible.', ephemeral: true });
    const allSuggestions = Array.from(suggestCmd.getAllSuggestions().values());
    if (allSuggestions.length === 0) return interaction.reply({ content: '🏆 Aucune suggestion.', ephemeral: true });
    const top = allSuggestions.sort((a, b) => b.upvotes.size - a.upvotes.size).slice(0, 10)
      .map((s, i) => `**${i + 1}.** #${s.id} - ${s.title?.substring(0, 40) || s.content.substring(0, 40)}... **(👍 ${s.upvotes.size})**`).join('\n');
    return interaction.reply({ embeds: [new EmbedBuilder().setColor('#FEE75C').setTitle('🏆 Top Suggestions').setDescription(top).setFooter({ text: 'Classement par votes positifs' })], ephemeral: true });
  }
  ...
  if (interaction.customId.startsWith('suggest_upvote_') || interaction.customId.startsWith('suggest_downvote_') || interaction.customId.startsWith('suggest_info_')) {
    ...
    if (action === 'info') {
      ...
      const embed = new EmbedBuilder()
        .setTitle(`ℹ️ Suggestion #${suggestionId}`).setColor('#5865F2')
        .addFields(
    ...
    const row = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`suggest_upvote_${suggestionId}`).setLabel(`${suggestion.upvotes.size}`).setEmoji('👍').setStyle(ButtonStyle.Success),
      new ButtonBuilder().setCustomId(`suggest_downvote_${suggestionId}`).setLabel(`${suggestion.downvotes.size}`).setEmoji('👎').setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId(`suggest_info_${suggestionId}`).setLabel('Infos').setEmoji('ℹ️').setStyle(ButtonStyle.Secondary)
    );
