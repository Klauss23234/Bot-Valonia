...
// =====================================================
const STAFF_ROLE_ID    = '1489722611297685514';       // 👈 ID du rôle staff (admin dans les tickets)
const SUPERIOR_ROLE_ID = '1489722614992601228';       // 👈 ID du rôle supérieur (responsable)
const LOG_CHANNEL_ID   = '1495873738690793614';       // 👈 ID du salon de logs/évaluations
...
const FILES = {
  ...
  claims:  './data/ticket-claims.json',
};
...
async function closeTicket(channel, guild, staff) {
  ...
  // Nettoyer le claim si existant
  const claims = readJSON(FILES.claims);
  if (claims[channel.id]) { delete claims[channel.id]; writeJSON(FILES.claims, claims); }
  ...
  async execute(message, args) {
    ...
    if (sub === 'setup') {
      ...
      const embed = new EmbedBuilder()
        .setTitle('🎫  Support — Besoin d\'aide ?')
        .setDescription([
          '**Notre équipe est disponible pour t\'aider !**',
          '',
          '─────────────────────────────',
          '📩  **Ouvre un ticket** en un clic',
          '📝  **Décris ta demande** clairement',
          '⚡  **Reçois une réponse** rapide',
          '─────────────────────────────',
          '',
          '> 💡 Merci de décrire ton problème **en détail** lors de l\'ouverture.',
        ...
        .addFields(
          { name: '🕐 Temps de réponse', value: 'Le plus vite possible',    inline: true },
          { name: '👥 Équipe',           value: `<@&${STAFF_ROLE_ID}>`,      inline: true },
          { name: '📋 Langues',          value: '🇫🇷 Français',               inline: true },
        )
        .setFooter({ text: `${message.guild.name} • Support`, iconURL: message.guild.iconURL({ dynamic: true }) ?? undefined })
        .setTimestamp();
    ...
    if (sub === 'rename') {
      ...
      if (!message.member.permissions.has(PermissionsBitField.Flags.ManageChannels))
        return message.reply('❌ Permission requise.');
      const newName = args.slice(1).join('-').toLowerCase().replace(/[^a-z0-9-]/g, '') || null;
      if (!newName) return message.reply('❌ Donne un nom : `+ticket rename mon-nouveau-nom`');
      await message.channel.setName(`ticket-${newName}`);
      return message.reply(`✅ Salon renommé en \`ticket-${newName}\``);
    }

    // 📌 Claim un ticket (commande texte)
    if (sub === 'claim') {
      if (!message.channel.name.startsWith('ticket-'))
        return message.reply('❌ Utilise cette commande dans un ticket.');
      if (!message.member.roles.cache.has(STAFF_ROLE_ID))
        return message.reply('❌ Réservé au staff.');
      await message.channel.send({ embeds: [new EmbedBuilder()
        .setDescription(`✅ Ce ticket a été **pris en charge** par ${message.member} — c'est lui qui gère ta demande !`)
        .setColor('#57F287')
        .setTimestamp()
      ]});
      await sendLog(message.guild, new EmbedBuilder()
        ...
        .setColor('#57F287')
        .addFields(
          { name: '🛡️ Staff',  value: `${message.member}`,          inline: true },
    ...
    if (sub === 'mute') {
      ...
      if (!targetId) return message.reply('❌ Mentionne un membre : `+ticket mute @membre`');
      await message.channel.permissionOverwrites.edit(targetId, { SendMessages: false });
      return message.reply(`✅ <@${targetId}> ne peut plus écrire dans ce ticket.`);
    }

    // 🔊 Unmute un membre dans le ticket
    if (sub === 'unmute') {
      if (!message.channel.name.startsWith('ticket-'))
        return message.reply('❌ Utilise cette commande dans un ticket.');
      if (!message.member.permissions.has(PermissionsBitField.Flags.ManageChannels))
        return message.reply('❌ Permission requise.');
      const targetId = args[1]?.replace(/\D/g, '');
      if (!targetId) return message.reply('❌ Mentionne un membre : `+ticket unmute @membre`');
      await message.channel.permissionOverwrites.edit(targetId, { SendMessages: true });
      return message.reply(`✅ <@${targetId}> peut à nouveau écrire dans ce ticket.`);
    }
    ...
    if (sub === 'transcript') {
      ...
      const msgList = [...fetched.values()].reverse();
      const attachment = new AttachmentBuilder(
        Buffer.from(generateHTML(msgList, message.channel.name), 'utf-8'),
        { name: `transcript-${message.channel.name}.html` }
      );
      await sendLog(message.guild, new EmbedBuilder()
        .setTitle('📋 Transcript manuel')
        .setColor('#5865F2')
        .addFields(
          { name: '🛡️ Demandé par', value: `${message.member}`,          inline: true },
          { name: '📌 Salon',       value: `\`${message.channel.name}\``, inline: true },
        ).setTimestamp()
      , [attachment]);
      return message.reply('✅ Transcript envoyé dans les logs !');
    }

    return message.reply('📋 Usage : `+ticket setup | close | stats [id] | rename [nom] | claim | mute @membre | unmute @membre | transcript`');
  }
};
...
export async function handleTicketButtons(interaction) {
    ...
    try {
      ...
      const ticketChannel = await guild.channels.create({
        ...
        permissionOverwrites: [
          ...
          // 🛡️ Le rôle staff = administrateur complet dans le ticket
          {
            id: STAFF_ROLE_ID,
            allow: [
              PermissionsBitField.Flags.ViewChannel,
      ...
      const welcomeEmbed = new EmbedBuilder()
        .setAuthor({
          name: `Ticket de ${member.user.username}`,
        ...
        .setDescription([
          ...
          '📎 N\'hésite pas à **préciser** ta demande en attendant.',
          '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
        ].join('\n'))
        .setColor('#5865F2')
        ...
        .addFields(
          ...
          { name: '🗓️ Ouvert le', value: `<t:${Math.floor(Date.now() / 1000)}:F>`,         inline: true },
          { name: '🆔 ID Membre', value: `\`${member.id}\``,                               inline: true },
          { name: '📌 Statut',    value: '🟡 En attente de prise en charge',               inline: false },
        )
        .setImage('https://i.imgur.com/4M34hi2.png')
        .setFooter({
          text: `${guild.name} • Support`,
          iconURL: guild.iconURL({ dynamic: true }) ?? undefined,
        })
        .setTimestamp();

      // Ligne 1 : appels + prise en charge
      const row1 = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          ...
          .setStyle(ButtonStyle.Primary),
        new ButtonBuilder()
          .setCustomId('call_superior')
          .setLabel('🔺 Appeler un supérieur')
      ...
      // Ligne 2 : gestion membres + mention + fermeture
      const row2 = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder()
          .setCustomId('remove_member_btn')
          .setLabel('➖ Retirer')
          .setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
          .setCustomId('mention_member_btn')
          .setLabel('📢 Mentionner')
          .setStyle(ButtonStyle.Secondary),
        new ButtonBuilder()
  ...
  // ── ✅ PRISE EN CHARGE DU TICKET (bouton claim) ──
  if (interaction.isButton() && interaction.customId === 'claim_ticket_btn') {
    // Seul le staff peut prendre en charge
    ...
    if (claims[channel.id]) {
      return interaction.reply({
        content: `❌ Ce ticket est déjà pris en charge par <@${claims[channel.id].staffId}> !`,
        ephemeral: true
      });
    }

    // Enregistrer le claim
    claims[channel.id] = { staffId: interaction.member.id, claimedAt: Date.now() };
    writeJSON(FILES.claims, claims);

    // Désactiver le bouton "Prendre en charge" sur le message d'origine
    try {
      ...
      await originalMsg.edit({ components: [disabledRow1, originalMsg.components[1]] }).catch(() => {});
    } catch {}

    // Annonce visible dans le ticket
    await interaction.reply({
      embeds: [new EmbedBuilder()
        .setTitle('✅ Ticket pris en charge')
        .setDescription([
          `${interaction.member} **s'occupe de ce ticket** !`,
          '',
          '> 🕐 Tu vas recevoir une réponse rapidement.',
          `> 🛡️ Staff assigné : ${interaction.member}`,
        ].join('\n'))
        .setColor('#57F287')
        .setThumbnail(interaction.member.user.displayAvatarURL({ dynamic: true }))
        .setFooter({ text: 'Pris en charge le' })
        .setTimestamp()
      ]
    });
    ...
    await sendLog(interaction.guild, new EmbedBuilder()
      ...
      .addFields(
        { name: '🛡️ Staff',  value: `${interaction.member} (${interaction.member.id})`, inline: true },
        { name: '📌 Salon',  value: `\`${channel.name}\``,                              inline: true },
        { name: '🕐 À',      value: `<t:${Math.floor(Date.now() / 1000)}:F>`,           inline: true },
      ).setTimestamp()
    );
  }
  ...
  // ── Mentionner le membre du ticket ──
  if (interaction.isButton() && interaction.customId === 'mention_member_btn') {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageChannels))
      return interaction.reply({ content: '❌ Réservé aux modérateurs.', ephemeral: true });

    const userId = interaction.channel.topic?.split('|')[0]?.trim() || '0';
    await interaction.reply({
      content: `📢 <@${userId}>, un membre du staff attire ton attention dans ce ticket !`,
    });
  }

  // ── Fermer le ticket ──
  if (interaction.isButton() && interaction.customId === 'close_ticket_btn') {
    const ticketOwnerId = interaction.channel.topic?.split('|')[0]?.trim() || '0';
    const isOwner = interaction.member.id === ticketOwnerId;
    const isStaff = interaction.member.permissions.has(PermissionsBitField.Flags.ManageChannels);

    if (!isOwner && !isStaff)
      return interaction.reply({ content: '❌ Seul le propriétaire du ticket ou le staff peut fermer ce ticket.', ephemeral: true });

    await interaction.reply({
      embeds: [new EmbedBuilder()
        .setTitle('⚠️ Confirmer la fermeture')
        .setDescription([
          'Es-tu sûr(e) de vouloir **fermer ce ticket** ?',
          '',
          '> 📋 Un transcript sera automatiquement généré.',
          '> ⭐ Tu pourras ensuite noter ton expérience.',
          '',
  ...
  if (interaction.isButton() && interaction.customId === 'close_ticket_cancel') {
    return interaction.update({ content: '↩️ Fermeture annulée.', embeds: [], components: [] });
  }
